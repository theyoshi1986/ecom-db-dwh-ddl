--Неотфактуровка. recadv без УПД
--sqlRecadvWithoutUpdCount
select
 a.varcompanyname
,count(1) cnt
	from dwh.bi_recadv_clean_x5_molvest rec
		left join dwh.bi_upd_dm_molvest upd
			on rec.recadv_ord_number=upd.upd_order_number
		   and (recadv_ord_date=upd.upd_order_date
		   	or upd.upd_order_date is null)
		   and rec.recadv_buyer_gln=upd.upd_recipient_gln
		   and rec.recadv_supplier_gln=upd.upd_sender_gln
		left join dwh.stg_gln gln
    		on gln.vargln=rec.recadv_buyer_gln
    	left join dwh.stg_account a
    		on a.intaccountid=gln.intaccountid
		where upd.intchainid is null
			and recadv_ord_date between ${parDtFrom} and ${parDtTo}
group by 1
order by 1;

--sqlRecadvWithoutUpdSumm
select
 a.varcompanyname
,round(sum(rec.ord_amount_recadv_calc),2) summ
	from dwh.bi_recadv_clean_x5_molvest rec
		left join dwh.bi_upd_dm_molvest upd
			on rec.recadv_ord_number=upd.upd_order_number
		   and (recadv_ord_date=upd.upd_order_date
...
,count(1) cnt
    from dwh.bi_upd_clean_molvest upd
		...
		where (upd_order_number is null
            ...
            and upd_date between ${parDtFrom} and ${parDtTo}
group by 1
order by 1;
...
select
 a.varcompanyname recepient
,round(sum(upd_amount_no_vat),2)/1000 summ
    from dwh.bi_upd_clean_molvest upd
    	left join dwh.stg_gln gln
    		on gln.vargln=upd.upd_recipient_gln
    	left join dwh.stg_account a
...
--sqlRecadvWithoutOrdSumm
select
 a.varcompanyname
,round(sum(coalesce(ord_amount_recadv_calc,0)),2)/1000 summ
	from dwh.bi_recadv_clean_x5_molvest rec
		left join dwh.bi_order_clean_x5_molvest ord
			on ord.ord_number=rec.recadv_ord_number
			and ord.ord_date=rec.recadv_ord_date
