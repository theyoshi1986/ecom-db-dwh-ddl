...
drop table dwh.bi_upd_position_clean_molvest;
create table dwh.bi_upd_position_clean_molvest
as
select *
	from (
		select 
		...
		,coalesce(upd_pos_vat,10) upd_pos_vat
			from dwh.bi_upd_position_trans_molvest trn
...
analyze verbose bi_upd_position_clean_molvest;

--Очищенные УПД
drop table dwh.bi_upd_clean_molvest;
create table dwh.bi_upd_clean_molvest
as
select 
 intdocid
,intchainid
...
,upd_pos_vat
	from dwh.bi_upd_position_clean_molvest
group by intdocid
,intchainid
,upd_number
,upd_date
...
analyze verbose dwh.bi_upd_clean_molvest;

--drop table dwh.bi_upd_dm_molvest;
create table dwh.bi_upd_dm_molvest
as
select 
 intchainid
,upd_number
...
analyze verbose bi_upd_clean_molvest;